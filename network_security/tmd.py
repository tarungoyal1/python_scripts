...
while True:
    ...
    fiborange = getLengthOfBinaryString(rawinput)

    hashdict = {}

    hash0 = {'0' : sha(rawinput)}
    hash1 = {'1' : sha(hash0)}

    hashdict.update(hash0)
    hashdict.update(hash1)
    count =2
    p=0
    q=1
    while True:
        nf = getNextFibonnaci(p,q)
        if nf>fiborange:
            break
        # print(str(nf)+"\n")
        hashdict.update({str(nf): str(sha(str(hashdict[str(p)]) + str(hashdict[str(q)])))})
        p=q
        q=nf
        tempnf = nf
    # print(hashdict)
    print(hashdict[str(tempnf)])
